"""
EkspertizBul — Konum Destekli Web Sunucusu
"""
...
FIRMS_DATA = [
    {"id":"otorapor","name":"Otorapor Ekspertiz","address":"Bağcılar, İstanbul","phone":"0850 XXX XX XX","website":"https://www.otorapor.com.tr","lat":41.0392,"lng":28.8562,"city":"İstanbul","certified":True,"rating":4.8,"reviews":1243},
    {"id":"autoking","name":"Auto King Ekspertiz","address":"Şişli, İstanbul","phone":"0212 XXX XX XX","website":"https://www.autoking.com.tr","lat":41.0602,"lng":28.9877,"city":"İstanbul","certified":True,"rating":4.6,"reviews":876},
    {"id":"dynomoss","name":"Dynomoss Oto Ekspertiz","address":"Kadıköy, İstanbul","phone":"0216 XXX XX XX","website":"https://dynomoss.com.tr","lat":40.9833,"lng":29.0333,"city":"İstanbul","certified":False,"rating":4.5,"reviews":654},
    {"id":"rs_ekspertiz","name":"RS Oto Ekspertiz","address":"Beşiktaş, İstanbul","phone":"0212 XXX XX XX","website":"https://rsotoekspertiz.com","lat":41.0430,"lng":29.0070,"city":"İstanbul","certified":True,"rating":4.3,"reviews":412},
    {"id":"arabam_ekspertiz","name":"Arabam.com Ekspertiz","address":"Maslak, İstanbul","phone":"0850 XXX XX XX","website":"https://www.arabam.com/oto-ekspertiz","lat":41.1057,"lng":29.0157,"city":"İstanbul","certified":True,"rating":4.9,"reviews":2108},
]
...
HTML = open(Path(__file__).parent / "templates" / "index.html").read() if (Path(__file__).parent / "templates" / "index.html").exists() else None
...
def index():
    ...
    return build_html(firms_json, prices_json)

def build_html(firms_json, prices_json):
...
.btn-g{{background:#00e5a0;border:none;cursor:pointer;padding:8px 16px;border-radius:8px;font-weight:600;font-size:.82rem;color:#000;text-decoration:none;transition:all .2s}}
...
<div class="container">
  <div class="sort-bar">
    ...
    <button class="sort-btn active" onclick="sortBy('distance',this)">📍 En Yakın</button>
    <button class="sort-btn" onclick="sortBy('price',this)">💰 En Ucuz</button>
    <button class="sort-btn" onclick="sortBy('rating',this)">⭐ En Yüksek Puan</button>
  </div>
  <div id="cards"></div>
</div>
<script>
...
let uLat=null,uLng=null,map=null,uMarker=null,sort='distance';
function initMap(){{
...
function getLocation(){{
  ...
  navigator.geolocation.getCurrentPosition(p=>{{
    ...
    uMarker=L.marker([uLat,uLng],{{icon:L.divIcon({{html:'<div style="background:#00e5a0;width:14px;height:14px;border-radius:50%;border:3px solid white;box-shadow:0 0 8px #00e5a0"></div>',iconSize:[14,14],iconAnchor:[7,7]}})}})).addTo(map).bindPopup('📍 Siz buradasınız').openPopup();
    renderCards();
  }},()=>{{document.getElementById('locText').textContent='Konum alınamadı — izin verin';renderCards();}});
}}
...
function sortBy(t,btn){{sort=t;document.querySelectorAll('.sort-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');renderCards();}}
function renderCards(){{
  let firms=FIRMS.map(f=>({{...f,distance:(uLat&&uLng)?dist(uLat,uLng,f.lat,f.lng):null,prices:PRICES[f.id]||[]}}));
  firms.sort((a,b)=>{{
    if(sort==='distance'){{if(!a.distance)return 1;if(!b.distance)return -1;return a.distance-b.distance;}}
    if(sort==='price'){{return(a.prices[0]?.price||99999)-(b.prices[0]?.price||99999);}}
    return b.rating-a.rating;
  }});
  document.getElementById('cards').innerHTML=firms.map((f,i)=>{{
    const near=i===0&&f.distance!==null;
    const dstr=f.distance!==null?f.distance.toFixed(1)+' km uzakta':'Konum alınmadı';
    const pkgs=f.prices.slice(0,3).map(p=>'<div class="pkg"><div class="pkg-n">'+p.name+'</div><div class="pkg-p">₺'+( p.price?.toLocaleString('tr')||'?')+'</div></div>').join('')||'<div class="pkg"><div class="pkg-n">Fiyat yükleniyor...</div></div>';
    return '<div class="card'+(near?' best':'')+'"><div class="card-top"><div><div class="firm-name">'+f.name+(near?'<span class="badge b-best">En Yakın</span>':'')+(f.certified?'<span class="badge b-cert">Sertifikalı</span>':'')+'</div><div class="firm-meta"><span>⭐ '+f.rating+' ('+f.reviews.toLocaleString('tr')+')</span><span>📍 '+f.address+'</span><span>📞 '+f.phone+'</span></div></div><div class="dist'+(near?' near':'')+'">📍 '+dstr+'</div></div><div class="pkgs">'+pkgs+'</div><div class="actions"><a href="'+f.website+'" target="_blank" class="btn-g">Randevu Al</a><button class="btn-w" onclick="goMap('+f.lat+','+f.lng+')">🗺️ Haritada Gör</button><button class="btn-w" onclick="yol('+f.lat+','+f.lng+')">🧭 Yol Tarifi</button></div></div>';
  }}).join('');
}}
function goMap(lat,lng){{map.setView([lat,lng],15);window.scrollTo({{top:0,behavior:'smooth'}});}}
function yol(lat,lng){{
  const url=uLat?'https://www.google.com/maps/dir/'+uLat+','+uLng+'/'+lat+','+lng:'https://www.google.com/maps/search/'+lat+','+lng;
